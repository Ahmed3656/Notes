...
- **Heap Fetches in Index Only Scans**: this happens when PostgreSQL can’t fully trust the index to tell whether a row is visible, so it goes back to the main table (heap) to check. The key reason is outdated visibility info, usually because a transaction has been running too long or vacuum hasn’t caught up. You need to run `VACUUM` to update the visibility map and clean out dead tuples, so future scans can stay index-only without the extra heap lookups.

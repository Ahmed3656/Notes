...
#### **How Data is Physically Stored on Disk: HDD vs. SSD**
This is about understanding the raw mechanics, which makes you appreciate the work the computer is doing for you. It’s like trying to peel off a price sticker from glass, you think it’ll come right off, but the sticky residue clings on.

- **The Basic Unit (Blocks):** whether on a spinning Hard Disk Drive (HDD) or a Solid State Drive (SSD), the disk is organized into **blocks** (typically 512 bytes or 4KB). The database never reads or writes a single row; it always operates on entire blocks.
- **Storing a Table:** imagine a simple table with one 4-byte integer field. When you insert rows, the database doesn't waste a whole block per row. Instead, it packs rows sequentially into a block. A 512-byte block can hold 128 rows (128 rows * 4 bytes = 512 bytes). The database's metadata acts like a map, pointing a row's ID to its specific block and offset.

<hr class="hr-light" />

**Hard Disk Drives (HDD):**
- **How it Works:** data is stored on spinning magnetic platters. To read data, a physical arm must move to the correct track and wait for the sector to spin under the head. This is governed by the laws of physics and is slow (seeking and rotational latency).
- **Addressing:** data is located by physical coordinates (e.g., Platter 1, Track 7, Sector 5).
- **Writes:** HDDs can **overwrite** data in place. You read a block, change the bytes in memory, and write the entire block back to the _exact same location_ on the disk. This is mechanically straightforward and doesn't degrade the disk.
	
	![HDD Structure](../images/hard-disk-structure.png)

<hr class="hr-light" />

**Solid State Drives (SSD):**
- **How it Works:** data is stored in flash memory cells with no moving parts. Access is electronic and random, making it hundreds of times faster than HDDs.
- **Addressing:** data is located by logical block numbers.
- **The Critical Difference:** no *in-place* writes. SSDs cannot overwrite existing data. To update a single row in a block:
    1. The entire block containing the row is read into memory.
    2. The row is updated in memory.
    3. The updated block must be written to a **new, fresh block** on the SSD. The old block is marked as stale and will be erased later.
- **Endurance:** this write/erase cycle has a physical limit per cell, known as **endurance**. Each cycle degrades the cell. Once a block wears out, it's "dead" and the SSD loses effective capacity.

<hr class="hr-light" />

**Impact on Database Structures ([B-Trees](../Data%20Structures/B-Trees.md) vs. [LSM Trees](../Data%20Structures/LSM%20Tree%20(Log-Structured%20Merge%20Tree).md)):**
- **The Problem with B-Trees on SSDs:** B-Trees are designed for HDDs. They frequently cause **in-place updates** during rebalancing operations (e.g., splitting nodes). On an SSD, this constant rewriting of index nodes **"kills the SSD's feeling"** and rapidly consumes its endurance.
- **The Solution: Log-Structured Merge-Trees (LSM Trees):** designed for modern hardware, LSM Trees are **append-only**. Instead of updating in place, they write changes to new, sequential segments. This is ideal for SSDs because it minimizes destructive overwrites and leverages their fast sequential write speed. This is why engines like **[RocksDB](./11.%20Database%20Engines.md)** were developed by companies like Facebook, which faced massive SSD wear from B-Trees.

**Understanding the physical layer, the difference between seeking on a spinning disk and the endurance of flash memory, is crucial for choosing the right data structures and database engines for your workload.**
...
- **Multiple Lock Levels:** databases use locks at different granularities - database, table, page, row
- **Lock Escalation:** when too many fine-grained locks are acquired, database may escalate to coarser locks to reduce memory overhead
- **Intent Locks:** higher-level locks that signal intention to lock at finer granularity (e.g., table-level intent lock before row-level locks)

**Lock Modes Beyond Basic Shared/Exclusive**
- **Update Locks:** intermediate state between shared and exclusive for read-modify-write operations
- **Schema Locks:** protect table structure during DDL operations
- **Bulk Update Locks:** special locks for bulk operations with different conflict rules

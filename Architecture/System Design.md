...
- **[DNS (Domain Name Server):](../Networking/DNS%20(Domain%20Name%20System).md)** translates human-readable domain names (e.g., `google.com`) into IP addresses (`142.250.181.206`) through a hierarchical, distributed system.
	- Core records: **A**, **AAAA**, **CNAME**, **NS**, **SOA**
	- DNS resolution involves recursive lookups from root -> TLD -> authoritative nameservers.
	- **Caching** is fundamental for performance (TTL defines how long results are cached).
	- Can provide **basic load balancing** (round-robin), and **global routing** via **Anycast**.
- **CDNs (Content Delivery Networks):** a geographically distributed network of servers that cache and deliver static content (images, CSS, JS, videos) closer to end-users.
...
- **[Sharding:](../Database/06.%20Sharding.md)** splitting a large database into smaller, independent shards stored on different servers. This improves performance, scalability, and query efficiency for massive datasets.
	**Challenges:**
    - Managing cross-shard queries.
    - Ensuring balanced data distribution.
    - Handling re-sharding when datasets grow unevenly.
- **[Replication:](../Database/08.%20Replication.md)** maintaining multiple synchronized copies of the same data across different servers.
	- **Types:**
